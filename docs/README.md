...
| [`design/lap-review-stage.md`](design/lap-review-stage.md) | An independent reading of what a lap produced, before its result is admitted as done: the material a person is shown at the gate, one refusal at `publish`, and a verdict that cannot certify what it never read. Decision rows `V-1` … `V-14`, **propose-only** — it takes no decision, and the entry it would create is drafted beside it. Its opening finding is that **`D-0019` rule 16's trigger has fired**: the rule refused the conductor's own verify because a failing check's only available action was the withdrawal ask, and `D-0025`'s operating surface gave rondo a second one. |
| [`design/lap-review-stage-decision-draft.md`](design/lap-review-stage-decision-draft.md) | The `DECISIONS.md` entry those rows would create, written out in full so the gate approves text rather than a promise of text. **Not part of the record** and not appended to `DECISIONS.md`; the ID it proposes (`D-0029`) is the next free one at the time of writing and moves if a concurrent belt claims it. |
