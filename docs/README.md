...
| [`design/advisory.md`](design/advisory.md) | The advisory component rondo#9 describes: a total, pure function from a durable snapshot to a **proposal** — an agent type, a `RunPlan`, a contract's candidate key sets, a widening successor — with its authority removed by construction. Decision rows `A-1` … `A-18`, **taken as `D-0022`**; the document itself is left as it was written, so a later reader can see what was measured and when. It measured the tree at `878eea4` and thirteen commits landed before the gate, so **`D-0022` governs wherever the two differ** — it re-measures `A-5`, `A-6` and `A-8`, and it **replaces `A-17`**: a lap-1 retry runs under a fresh derived triple and widens by taking a fresh plan, not by adopting a successor contract. |
| [`design/lap-review-stage.md`](design/lap-review-stage.md) | An independent reading of what a lap produced, before its result is admitted as done: the material a person is shown at the gate, one refusal at `publish`, and a verdict that cannot certify what it never read. Decision rows `V-1` … `V-14`, **taken as `D-0029`**; the document is left as it was written, so a later reader can see what was measured and when. It measured the tree at `0497ca8` and every citation was re-taken at `7c63f05` before the gate, so **`D-0029` governs wherever the two differ** — it sharpens one citation, `inspectLapWork`'s tip query, and moves no decision. Its opening finding is that **`D-0019` rule 16's trigger has fired in its second limb**: the rule refused the conductor's own verify because a failing check's only available action was the withdrawal ask, and `D-0025`'s operating surface gave rondo a second one. The entry the rows would create was drafted beside the document and **removed once appended**, because a second copy of an accepted decision has no rule for which copy wins. |
| [`design/parallel-admission.md`](design/parallel-admission.md) | The two things `D-0012` says parallel admission waits on that are rondo's: an **allocator** for the (run id, topic branch, workspace) triple, and a **capacity ledger** replacing `R-10`'s partial unique index. Paired with continuo #167, which owns the delivery-lease half. Decision rows `N-1` … `N-28`, **taken as `D-0023`**; the document itself is left as it was written, so a later reader can see what was measured and when. |
| [`design/refusal-session-lock.md`](design/refusal-session-lock.md) | What `D-0019` rule 11 rests on, now that a refusal can name a session (`continuo D-1102`, rondo#24): whether a `performing` iteration whose lap refused should keep the single-flight lock until a stop is confirmed. Measured against the pinned continuo, it finds rule 11's conclusion sound for a reason the rule does not state — a live child holds `lap perform` open, so a refusal that *reaches* rondo is a process that already exited — and finds the real gap on the `defect` path instead. Decision rows `S-1` … `S-6`, of which `S-1` … `S-4` are **taken as `D-0035`**; the document itself is left as it was written, so a later reader can see what was measured and when, and `D-0035` governs wherever the two differ. `S-5` became rondo#55 and `S-6` is an ask filed against continuo. |
...
| A reading that could not be taken can never be recorded as `clear`, so a lap whose work nobody could read cannot publish as though it had been (`D-0029` rules 10 and 12) | [`../test/access/review.test.ts`](../test/access/review.test.ts), and the `review-is-not-vacuous` job — check name "a fail-open reading makes the review test fail" — in [`../.github/workflows/ci.yml`](../.github/workflows/ci.yml), which plants the fail-open and requires the named case to be what goes red |
| A `clear` verdict is only ever written beside rondo's own measurement of what was read, and a reading lands in the same transaction as the transition it accompanies (`D-0029` rules 8 and 11) | [`../test/store/reading.test.ts`](../test/store/reading.test.ts) |
...
| The Windows cells run on the nightly schedule and on demand and never on the pull-request path, and are not quietly retired from the matrix either (`D-0087`) | the `os:` expression of `double-green` in [`../.github/workflows/ci.yml`](../.github/workflows/ci.yml), asserted from both sides by [`../test/architecture/ci-triggers.test.ts`](../test/architecture/ci-triggers.test.ts) |
| A red on that path becomes an issue rather than a row in a tab, and the token that can write one is held by that job alone (`D-0087` rule 7) | the `nightly-red-is-an-issue` job in [`../.github/workflows/ci.yml`](../.github/workflows/ci.yml), asserted by [`../test/architecture/ci-triggers.test.ts`](../test/architecture/ci-triggers.test.ts) |
