 ...
 *
 * So this reads Node's `errno` where there is one, and nothing else:
 *
...
/** The errno this error carries and this module vouches for, or null. */
function hostSetupCode(error: unknown): HostSetupCode | null {
  ...
  // crossing a worker or a `node:sqlite` fault can arrive as a plain object
  // with a `code`, and refusing those would put a real ENOENT in `unknown`.
  const code = (error as { readonly code?: unknown } | null)?.code;
  return HOST_SETUP_CODES.find((known) => known === code) ?? null;
}
